...
export function IndexCard({ credential }: { credential: LlmCredentialDto }) {
  ...
  return (
    <Panel className="flex flex-col gap-4 p-5 sm:p-6">
      ...
      {sources.length > 0 ? (
        <p className="flex flex-wrap items-center gap-x-4 gap-y-1 font-mono text-[11px] text-warm-slate">
      ...
      {sources.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded border border-warm-border">
          <table className="w-full min-w-[34rem] border-collapse text-left">
            <thead>
              <tr className="border-b border-warm-border bg-warm-sunken font-mono text-[10.5px] tracking-wider text-warm-slate uppercase">
...
function SourceRow({ source }: { source: RagSourceDto }) {
  ...
  const chip = (
    <span
      className={cn(
        "inline-flex items-center gap-1 rounded border px-1.5 py-0.5 font-mono text-[10.5px] font-medium",
        copy.tone,
      )}
    >
      <Icon
      ...
      {copy.label}
      {isFailed ? (
        <Icon name="chevron-down" className={cn("text-[12px] transition-transform", open && "rotate-180")} />
      ) : null}
    </span>
  );
  ...
  return (
    <>
      <tr className="border-b border-warm-hairline last:border-b-0">
        <td className="max-w-[16rem] truncate px-3 py-2 text-[13px] text-warm-black" title={source.label}>
          {source.label || "Untitled"}
        </td>
        <td className="px-3 py-2 font-mono text-[11px] text-warm-slate">
        ...
        <td className="px-3 py-2 text-right font-mono text-[11px] text-warm-black tabular-nums">
          {status === RagSourceStatus.Indexed || (source.passageCount ?? 0) > 0
            ? (source.passageCount ?? 0).toLocaleString()
            : "—"}
        </td>
        <td className="px-3 py-2 text-right font-mono text-[11px] text-warm-slate">
      ...
      {isFailed && open ? (
        <tr className="border-b border-warm-hairline bg-warm-danger-bg/60 last:border-b-0">
          <td colSpan={5} className="px-3 py-2.5 text-[12.5px] leading-relaxed text-warm-danger">
            {source.error ?? "The worker did not say why."}
            <span className="mt-1 block font-mono text-[10.5px] text-warm-slate">
