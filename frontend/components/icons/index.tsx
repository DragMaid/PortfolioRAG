...
/**
 * A monogram in a rounded square, for a service with no mark of its own here.
 *
 * Drawing a rough approximation of somebody's logo reads worse than not drawing it: a
 * wrong Mastodon elephant is noticeably wrong, whereas a lettermark is plainly a
 * lettermark. It also means an unrecognised link still gets something that identifies it.
 */
export function LetterMark({ letter, ...props }: IconProps & { letter: string }) {
  return (
...
/**
 * The mark for each service the API can detect. A kind missing from here falls through to
 * a lettermark, which is what the four with hand-drawn-only logos get.
 */
const contactIcons: Partial<Record<ContactIcon, (props: IconProps) => React.ReactElement>> = {
  Website: GlobeIcon,
  Email: MailIcon,
  Phone: PhoneIcon,
  GitHub: GitHubIcon,
...
/** The letter shown for a service with no mark above. */
const contactLetters: Partial<Record<ContactIcon, string>> = {
  Mastodon: "M",
  Threads: "@",
  Behance: "B",
  DevTo: "D",
...
/** Resolves a `ContactLink.icon` discriminator to its mark. */
export function ContactGlyph({ name, ...props }: IconProps & { name: ContactIcon }) {
  const Glyph = contactIcons[name];
  if (Glyph) return <Glyph {...props} />;

  return <LetterMark letter={contactLetters[name] ?? name.charAt(0)} {...props} />;
}
